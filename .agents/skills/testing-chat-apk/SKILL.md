...
- `CF_API_KEY`, `CF_ACCOUNT_ID`, `CF_GATEWAY_NAME`, `CF_MODEL`
...
### Chat Mode (always works with Cloudflare API)
...
3. **Response streaming**: AI response streams in real-time from Cloudflare Workers AI, displayed without bubble on left side
...
- Backend: Express server at `server/routes.ts` with `/api/chat` (Cloudflare SSE proxy) and `/api/agent` (Python process spawner)
